...
EVAL_DOWN_STEPS="${EVAL_DOWN_STEPS:-4}"
GLOBAL_SEED="${GLOBAL_SEED:-43}"

RUN_TAG="${RUN_TAG:-step1_${FEATURE_MAP_TYPE}_qwen3_1p7b_ms${MAX_SAMPLES}_seed${GLOBAL_SEED}}"
RUN_ROOT="${RUN_ROOT:-/root/autodl-tmp/Energy/ebft_openrlhf_stepwise/runs/${RUN_TAG}}"
...
echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] Starting Step 1 feature-map run"
...
echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] Step 1 feature-map run finished"
