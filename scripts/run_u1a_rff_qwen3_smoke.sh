...
RUN_ROOT="${RUN_ROOT:-/root/autodl-tmp/Energy/ebft_openrlhf_stepwise/runs/u1a_rff_qwen3_smoke}"
SAVE_PATH="${SAVE_PATH:-${RUN_ROOT}/model}"
TB_ROOT="${TB_ROOT:-${RUN_ROOT}/tensorboard}"
RUN_NAME="${RUN_NAME:-u1a_rff_qwen3_smoke}"

mkdir -p "${RUN_ROOT}" "${SAVE_PATH}" "${TB_ROOT}"

exec > >(tee -a "${RUN_ROOT}/train.log") 2>&1

echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] Starting U1-A fixed-RFF smoke run"
...
echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] U1-A fixed-RFF smoke run finished"
