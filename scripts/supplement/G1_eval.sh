...
RUN_DIR="${RUN_DIR:-${1:-}}"
if [[ -z "${RUN_DIR}" ]]; then
  echo "Usage: RUN_DIR=/path/to/run bash scripts/supplement/G1_eval.sh"
  echo "   or: bash scripts/supplement/G1_eval.sh /path/to/run"
  exit 1
fi
...
POST_EVAL_MASTER_PORT="${POST_EVAL_MASTER_PORT:-29514}"
