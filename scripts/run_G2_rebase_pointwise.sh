#!/usr/bin/env bash
# ╔══════════════════════════════════════════════════════════════════╗
# ║  Experiment C Rebase — teacher + pointwise                      ║
# ║  fixed local-vLLM teacher · pointwise reward only               ║
# ╚══════════════════════════════════════════════════════════════════╝
#
# Based on: scripts/run_G2_rebase.sh
#
# Intended training-side differences vs G2 only:
#   1) --distribution_reward_type pointwise
#   2) do NOT pass any --cf_* arguments
#
# Everything else is kept identical to G2:
#   - local vLLM teacher launch / API / cache / prefetch switches
#   - dataset / model / batch / rollout / seed / optimizer / lr
#   - teacher sampling temperature / top_p / max_new_tokens / timeout
#
# This is the "clean distribution matching ablation" pointwise arm:
# reward uses pointwise alignment + diversity only, without CF-based
# distribution matching.
#
# Override examples:
#   TARGET_STEPS=500 bash scripts/run_G2_rebase_pointwise.sh
#   MAX_SAMPLES=10000 bash scripts/run_G2_rebase_pointwise.sh
set -euo pipefail
...
# --------------------------------------------------------------------
# 0) MANUAL GPU ASSIGNMENT (same as G2)
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# 1) TEACHER SERVING CONFIG (local vLLM) — identical to G2
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# 2) TRAINING DATA / MODEL PATHS — identical to G2
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# 3) TRAINING KNOBS — identical to G2
# --------------------------------------------------------------------
...
# Kept identical to G2 for auditability, but intentionally NOT forwarded
# as --cf_* flags in this pointwise ablation.
...
RUN_NAME="${RUN_NAME:-ablation_teacher_pointwise_rebase_$(date +%m%d_%H%M)}"
...
echo "========== Experiment C Rebase (teacher + pointwise) =========="
...
echo "Teacher target M (audit):   ${CF_TEACHER_N_SAMPLES}"
...
# UNIQUE DIFF 1/2 vs run_G2_rebase.sh:
#   --distribution_reward_type pointwise
# UNIQUE DIFF 2/2 vs run_G2_rebase.sh:
#   no --cf_* arguments are passed
...
echo "──────────────────────────────────────────────────"
echo "teacher + pointwise rebase run completed at $(date)" > "${RUN_DIR}/status.txt"
