#!/usr/bin/env bash
# G1 Formal - 真正的 8 卡训练: actor=4, critic=4, 使用处理后的 AoPS 数据
set -euo pipefail
...
RUN_NAME="g1_formal_8gpu_final_$(date +%m%d_%H%M)"
RUN_DIR="/mnt/workspace/runs/$RUN_NAME"
mkdir -p "$RUN_DIR"

cd "$REPO_ROOT"

echo "=== G1 Formal 8GPU (4+4) started at $(date) ===" | tee "$RUN_DIR/train.log"
echo "Run dir: $RUN_DIR" | tee -a "$RUN_DIR/train.log"
echo "Data: $TRAIN_DATA (question/answer format)" | tee -a "$RUN_DIR/train.log"

# G1 Formal: pointwise, 4+4 拓扑，使用正确的数据路径
