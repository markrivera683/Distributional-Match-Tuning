...
# Reference warm-start SFT validation hyperparameters from the paper.
# This script assumes the warm-start checkpoint at SFT_SAVE_PATH already exists.
REF_SFT_TRAIN_BATCH_SIZE="${REF_SFT_TRAIN_BATCH_SIZE:-64}"
REF_SFT_MAX_EPOCHS="${REF_SFT_MAX_EPOCHS:-1}"
REF_SFT_MAX_LEN="${REF_SFT_MAX_LEN:-2048}"
REF_SFT_LR="${REF_SFT_LR:-1e-5}"
REF_SFT_LR_SCHEDULER="${REF_SFT_LR_SCHEDULER:-cosine_with_min_lr}"
REF_SFT_WARMUP_RATIO="${REF_SFT_WARMUP_RATIO:-0.03}"

# Downstream validation sets / protocol (documented here for reproducibility).
...
EBFT_MAX_SAMPLES="${EBFT_MAX_SAMPLES:-100000}"
...
RUN_POST_EBFT_BENCHMARKS="${RUN_POST_EBFT_BENCHMARKS:-true}"
...
if ! is_local_resource "${MODEL_PATH}" || ! is_local_resource "${SFT_SAVE_PATH}"; then
...
# populated.  To download them for the first time, temporarily run:
#   HF_HUB_OFFLINE=0 HF_DATASETS_OFFLINE=0 bash scripts/reproduction_ebft_100k.sh
...
RUN_NAME="${RUN_NAME:-repro_ebft_100k_qwen25_1p5b_$(date +%m%d_%H%M)}"
...
mkdir -p "${RUN_DIR}" "${EBFT_SAVE_PATH}" "${EBFT_CKPT_PATH}" "${EBFT_TB_DIR}"
...
if [[ -z "${SFT_SAVE_PATH}" ]]; then
  echo "[ERROR] SFT_SAVE_PATH is empty."
  echo "        Example:"
  echo "        SFT_SAVE_PATH=/root/outputs/<run>/stage1_sft_model bash scripts/reproduction_ebft_100k.sh"
  exit 1
fi
...
if is_local_resource "${SFT_SAVE_PATH}" && ! path_spec_exists "${SFT_SAVE_PATH}"; then
  echo "[ERROR] SFT_SAVE_PATH not found: ${SFT_SAVE_PATH}"
  exit 1
fi

if is_local_resource "${SFT_SAVE_PATH}" && ! has_model_weights "${SFT_SAVE_PATH}"; then
  echo "[ERROR] SFT_SAVE_PATH exists but no warm-start weights were found: ${SFT_SAVE_PATH}"
  exit 1
fi

if [[ "${HF_HUB_OFFLINE}" == "1" ]] && (! is_local_resource "${MODEL_PATH}" || ! is_local_resource "${SFT_SAVE_PATH}"); then
  echo "[ERROR] HF_HUB_OFFLINE=1 but MODEL_PATH/SFT_SAVE_PATH use a remote repo id."
  echo "        Use local checkpoints or set HF_HUB_OFFLINE=0."
  exit 1
fi
...
if [[ "${RUN_POST_EBFT_BENCHMARKS}" == "true" ]]; then
...
if (( EBFT_TRAIN_BATCH_SIZE % (EBFT_MICRO_TRAIN_BATCH_SIZE * ACTOR_GPUS) != 0 )); then
  echo "[ERROR] EBFT train_batch_size must be divisible by micro_train_batch_size * actor_gpus"
  echo "        ${EBFT_TRAIN_BATCH_SIZE} % (${EBFT_MICRO_TRAIN_BATCH_SIZE} * ${ACTOR_GPUS}) != 0"
  exit 1
fi
...
if (( EBFT_MAX_SAMPLES <= 0 )); then
  echo "[ERROR] EBFT_MAX_SAMPLES must be > 0"
  exit 1
fi

echo "========== Reproduction: EBFT 100k =========="
echo "RUN_DIR:                     ${RUN_DIR}"
echo "CUDA_VISIBLE_DEVICES:        ${CUDA_VISIBLE_DEVICES} (count=${gpu_count})"
echo "Actor/Critic GPUs:           ${ACTOR_GPUS}/${CRITIC_GPUS}"
echo "Warm-start model:            ${SFT_SAVE_PATH}"
echo "Base model / critic init:    ${MODEL_PATH}"
...
echo "Post EBFT benchmarks:        ${RUN_POST_EBFT_BENCHMARKS}"
...
echo ""
echo "[Reference Stage 1] SFT warm-start"
echo "  train_batch_size:          ${REF_SFT_TRAIN_BATCH_SIZE}"
echo "  max_epochs:                ${REF_SFT_MAX_EPOCHS}"
echo "  max_len:                   ${REF_SFT_MAX_LEN}"
echo "  learning_rate:             ${REF_SFT_LR}"
echo "  lr_scheduler:              ${REF_SFT_LR_SCHEDULER}"
echo "  warmup_ratio:              ${REF_SFT_WARMUP_RATIO}"
echo ""
echo "[Stage 2] EBFT"
...
run_code_benchmarks() {
  ...
  mkdir -p "${bench_dir}"
  echo ""
  echo "===== EBFT 100k: downstream code benchmarks ====="
...
ebft_cmd=(
  ...
  --wandb_run_name "${RUN_NAME}_ebft_100k"
)
...
if [[ "${RUN_POST_EBFT_BENCHMARKS}" == "true" ]]; then
  run_code_benchmarks "${EBFT_SAVE_PATH}" "${RUN_DIR}/benchmarks_stage2"
fi

echo ""
echo "===== Finished ====="
echo "Run dir:        ${RUN_DIR}"
echo "Warm-start:     ${SFT_SAVE_PATH}"
