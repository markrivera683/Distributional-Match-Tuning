...
# For each model and each benchmark (HumanEval, MBPP), run 3 repeats:
#   1) greedy
#   2) sample pass@1 with temperature=0.6
  ...
  case "${task_name}" in
    greedy)
      task_args=(
        ...
        --sample_temperature "${PASS1_TEMPERATURE}"
        --n_samples 1
        --passk_list 1
      )
      ;;
...
RUN_NAME="${RUN_NAME:-baseline_g1_g2_g3_code_eval_repeats_$(date +%m%d_%H%M)}"
...
PASS1_TEMPERATURE="${PASS1_TEMPERATURE:-0.6}"
TOP_P="${TOP_P:-1.0}"
...
echo "sample pass@1 temp:         ${PASS1_TEMPERATURE}"
echo "======================================="
