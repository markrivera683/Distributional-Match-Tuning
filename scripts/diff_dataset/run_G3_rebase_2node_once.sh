...
#       trainable critic head + classifier loss + direct discrepancy +
#       diversity/alignment reward + ce_loss.
#
# Same DLC / DSW-SSH / single-node mode autodetection as run_G2_rebase_2node_once.sh.
# Same 6-teacher / 2-student GPU split per node (head 6t+2s, worker 6t+2s).
# Post-train eval uses head node only at TP=8 via supplement_2rounds/G3.sh.
#
# Assumptions:
...
if [[ "${DLC_MODE}" == "true" ]]; then
  ...
  echo "Examples:"
  echo "  HEAD_NODE=node0 WORKER_NODE=node1 bash scripts/run_G3_rebase_2node_once.sh   # DSW ssh"
  echo "  bash scripts/run_G3_rebase_2node_once.sh                                     # single-node / DLC"
  exit 1
fi
...
#   Why default true: CF_TEACHER_N_SAMPLES (default 8) means each unique
#   prompt is asked to the teacher 8 times in quick succession. With prefix
#   caching ON, only the first call pays the prefill cost; the other 7 hit
#   the prefix KV cache and skip prefill entirely. On a 27B teacher this
#   wins 3-5x on per-prompt teacher latency for the repeated-sample workload.
#   Set TEACHER_ENABLE_PREFIX_CACHING=false to opt out (e.g. when debugging
#   a vLLM correctness issue).
...
TEACHER_REMOTE_BATCH_SIZE="${TEACHER_REMOTE_BATCH_SIZE:-96}"
...
# IN-TRAINING EVAL DISABLED BY DEFAULT (mirrors run_G2_rebase_2node_once.sh).
#   GENERATE_MAX_LEN=8 (the EBFT token-level rollout) makes in-training
#   eval generate only 8 tokens, which is meaningless for code tasks and
#   clutters TensorBoard. The post-training code benchmark eval below is
#   what we trust for MBPP / HumanEval accuracy reporting.
#
#   To re-enable in-training eval:
#       EVAL_STEPS=200 EVAL_GENERATE_MAX_LEN=2048 bash scripts/run_G3_rebase_2node_once.sh
...
# Post-training offline code benchmark eval (16k -> 32k via vLLM in
# scripts/supplement_2rounds/G3.sh). Aligned with G1/G2/baseline so all
...
POST_EVAL_SCRIPT="${POST_EVAL_SCRIPT:-}"
...
# vLLM concurrency knobs. See scripts/supplement_2rounds/G3_2node.sh for the
...
ARCHIVE_OUTPUT_ROOT="${ARCHIVE_OUTPUT_ROOT:-/mnt/data/ebft-teacher-distribution/outputs_diff_dataset_g3}"
...
#     alpha = 0.5  -> diversity penalty halved (the previous default)
#     alpha = 0    -> alignment-only, mode-seeking / unstable
#   Paper gamma (CE-loss weight) = CE_LOSS_COEF (defined above, default 0.03)
#
# Default here matches paper's standard configuration: alpha=1, gamma=0.03.
...
# (e.g. dlcXXXXXX-master-0 -> g3_dlcXXXXXX) so master and worker pods
...
if [[ "${DLC_MODE}" == "true" && -z "${RUN_NAME:-}" ]]; then
  ...
  if [[ -n "${_dlc_job_id}" && "${_dlc_job_id}" != "$(hostname)" ]]; then
    RUN_NAME="diff_g3_${_dlc_job_id}"
  fi
fi
RUN_NAME="${RUN_NAME:-diff_g3_qwen35_4b_2node_$(date +%m%d_%H%M)}"
...
write_run_metadata() {
  local vars=(
    ...
    CODE_EVAL_N_SAMPLES CODE_EVAL_PASSK_LIST CODE_EVAL_GREEDY_BATCH_SIZE CODE_EVAL_SAMPLE_BATCH_SIZE
    MBPP_EVAL_DATA HUMANEVAL_EVAL_DATA MBPP_EVAL_CONFIG MBPP_EVAL_SPLIT HUMANEVAL_EVAL_SPLIT
...
if [[ "${RUN_TWO_ROUND_EVAL}" == "true" ]]; then
  if [[ ! -x "${TEACHER_PYTHON_BIN}" ]]; then
    ...
    echo "        (needed by code benchmark vLLM eval; set RUN_TWO_ROUND_EVAL=false to skip)"
    exit 1
  fi
  if [[ ! -x "${CODE_BENCHMARK_PYTHON_BIN}" ]]; then
    echo "[ERROR] CODE_BENCHMARK_PYTHON_BIN not executable: ${CODE_BENCHMARK_PYTHON_BIN}"
    exit 1
  fi
...
echo "Code benchmark script:      ${CODE_BENCHMARK_SCRIPT}"
echo "Post-eval VLLM_TP_SIZE:     ${VLLM_TP_SIZE} (head node only; worker idle during eval)"
echo "Code eval max_new_tokens:   ${CODE_EVAL_MAX_NEW_TOKENS}"
...
if [[ "${RUN_TWO_ROUND_EVAL}" == "true" ]]; then
  if (( TRAIN_RC == 0 )); then
    echo ""
    echo "===== Running MBPP + HumanEval code benchmark eval (head node, 8 GPUs) ====="
    ...
    export CODE_BENCHMARK_SCRIPT CODE_BENCHMARK_PYTHON_BIN CODE_BENCHMARKS CODE_BENCHMARK_BACKEND
    ...
    export CODE_EVAL_N_SAMPLES CODE_EVAL_PASSK_LIST CODE_EVAL_GREEDY_BATCH_SIZE CODE_EVAL_SAMPLE_BATCH_SIZE
    export MBPP_EVAL_DATA HUMANEVAL_EVAL_DATA MBPP_EVAL_CONFIG MBPP_EVAL_SPLIT HUMANEVAL_EVAL_SPLIT
    ...
    # Tell DLC worker pod (if any) that post-eval is complete so it can exit.
    if [[ "${DLC_MODE}" == "true" ]]; then
      _rdv_helper_path="${REPO_ROOT}/scripts/supplement_2rounds/_rendezvous_dlc.sh"
      if [[ -f "${_rdv_helper_path}" ]]; then
        # shellcheck disable=SC1090
