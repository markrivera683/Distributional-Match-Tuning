...
print_teacher_model_path_hint() {
  cat >&2 <<'EOF'
        Expected a local HuggingFace checkpoint directory.
        Common DLC/DSW locations:
          /mnt/data/models/Qwen3.5-27B
          /mnt/data/models/qwen3.5-27b
          /mnt/data/teacher_model/models/Qwen3.5-27B
          /mnt/data/teacher_model/models/qwen3.5-27b
        Override with: TEACHER_MODEL_PATH=/path/to/model
EOF
}
...
ARCHIVE_OUTPUT_ROOT="${ARCHIVE_OUTPUT_ROOT:-/mnt/data/ebft-teacher-distribution/outputs_diff_dataset_g2}"
...
if [[ ! -d "${TEACHER_MODEL_PATH}" ]]; then
  echo "[ERROR] TEACHER_MODEL_PATH not found: ${TEACHER_MODEL_PATH}" >&2
  print_teacher_model_path_hint
  exit 1
fi
if [[ ! -f "${TEACHER_MODEL_PATH}/config.json" ]]; then
  echo "[ERROR] TEACHER_MODEL_PATH does not look like a HuggingFace checkpoint: ${TEACHER_MODEL_PATH}" >&2
  echo "        missing ${TEACHER_MODEL_PATH}/config.json" >&2
  print_teacher_model_path_hint
  exit 1
fi
...
echo "Max samples:                ${MAX_SAMPLES} (-1 means full train split)"
...
wait_for_http_health() {
  ...
  port="${url##*:}"
  port="${port%%/*}"
  idx=$((port - TEACHER_BASE_PORT))
  until curl -sf "${url%/v1}/health" >/dev/null; do
    sleep 3
    waited=$((waited + 3))
    if (( idx >= 0 && idx < ${#LOCAL_TEACHER_PIDS[@]} )); then
      pid="${LOCAL_TEACHER_PIDS[$idx]}"
      if [[ -n "${pid}" ]] && ! kill -0 "${pid}" 2>/dev/null; then
        echo "[ERROR] local teacher process for ${url} exited before health check passed (pid=${pid})"
        echo "        check ${TEACHER_LOG_DIR}/worker_${HEAD_NODE}_${idx}.log"
        return 1
      fi
    fi
...
dlc_worker_bootstrap() {
  ...
  local w port gpu_start g worker_gpus log_file teacher_pid
  local _dlc_teacher_pids=()
  for (( w=0; w<WORKER_TEACHER_WORKER_COUNT; w++ )); do
    ...
    " > "${log_file}" 2>&1 &
    teacher_pid=$!
    _dlc_teacher_pids+=("${teacher_pid}")
    echo "${teacher_pid}" > "${PID_DIR}/dlc_worker_teacher_${w}.pid"
  ...
  until ray status --address "${DLC_MASTER_ADDR}:${RAY_PORT}" >/dev/null 2>&1; do
    sleep 5
    waited=$((waited + 5))
    for (( w=0; w<${#_dlc_teacher_pids[@]}; w++ )); do
      if ! kill -0 "${_dlc_teacher_pids[$w]}" 2>/dev/null; then
        echo "[DLC worker] ERROR: teacher #${w} exited before master ray head came up (pid=${_dlc_teacher_pids[$w]})"
        echo "                 check ${TEACHER_LOG_DIR}/worker_dlc_rank${DLC_NODE_RANK}_${w}.log"
        exit 1
      fi
    done
