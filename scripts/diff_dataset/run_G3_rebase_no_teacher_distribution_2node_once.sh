#!/usr/bin/env bash
# Standalone diff-dataset G3 no-teacher EMA0.99 + DISTRIBUTION 2-node launcher.
#
...
#          teacher cache, no teacher prefetch.
# G3:      EMA=0.99 + feature_adapter + trainable critic head + direct discrepancy.
...
ARCHIVE_OUTPUT_ROOT="${ARCHIVE_OUTPUT_ROOT:-/mnt/data/ebft-teacher-distribution/outputs_diff_dataset_g3_no_teacher_distribution}"
...
if [[ "${DLC_MODE}" == "true" && -z "${RUN_NAME:-}" ]]; then
  ...
  if [[ -n "${_dlc_job_id}" && "${_dlc_job_id}" != "$(hostname)" ]]; then
    RUN_NAME="diff_g3_no_teacher_distribution_ema099_${_dlc_job_id}"
  fi
fi
if [[ -z "${RUN_NAME:-}" && "${DLC_MODE}" != "true" ]]; then
  RUN_NAME="diff_g3_no_teacher_distribution_ema099_qwen35_4b_2node_$(date +%m%d_%H%M)"
fi
RUN_NAME="${RUN_NAME:-diff_g3_no_teacher_distribution_ema099_qwen35_4b_2node_$(date +%m%d_%H%M)}"
