#!/usr/bin/env bash
# Stage 3: async slime run using the Stage 2 pointwise reward configuration.
# train_async.py does not support colocate, so actor and rollout GPUs are split.
...
export TRAIN_DRIVER="${TRAIN_DRIVER:-train_async.py}"
export SLIME_ROOT
...
export RUN_NAME="${RUN_NAME:-slime_async_g1_pointwise_qwen35_4b_$(date +%m%d_%H%M)}"
