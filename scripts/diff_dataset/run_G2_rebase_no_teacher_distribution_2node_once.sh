#!/usr/bin/env bash
# Standalone diff-dataset G2 no-teacher DISTRIBUTION-only 2-node launcher.
#
# Dataset: OpenCodeInstruct train pool, MBPP + HumanEval post-eval.
# Model:   Qwen3.5-4B student by default.
# Reward:  cf_l1oo with cf_target_mode=single. No teacher process, no
#          vicinal target smoothing, no teacher cache, no teacher prefetch.
...
ARCHIVE_OUTPUT_ROOT="${ARCHIVE_OUTPUT_ROOT:-/mnt/data/ebft-teacher-distribution/outputs_diff_dataset_g2_no_teacher_distribution}"
...
if [[ "${DLC_MODE}" == "true" && -z "${RUN_NAME:-}" ]]; then
  ...
  if [[ -n "${_dlc_job_id}" && "${_dlc_job_id}" != "$(hostname)" ]]; then
    RUN_NAME="diff_g2_no_teacher_distribution_${_dlc_job_id}"
  fi
fi
if [[ -z "${RUN_NAME:-}" && "${DLC_MODE}" != "true" ]]; then
  RUN_NAME="diff_g2_no_teacher_distribution_qwen35_4b_2node_$(date +%m%d_%H%M)"
fi
RUN_NAME="${RUN_NAME:-diff_g2_no_teacher_distribution_qwen35_4b_2node_$(date +%m%d_%H%M)}"
