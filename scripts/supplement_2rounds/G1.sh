#!/usr/bin/env bash
# G1 two-round post-eval via vLLM: 16k first pass, 32k retry on incorrect prompts.
# Usage:
#   bash scripts/supplement_2rounds/G1.sh /root/outputs/g1_rebase_0405_2259
#   RUN_DIR=/root/outputs/g1_rebase_0405_2259 bash scripts/supplement_2rounds/G1.sh
set -euo pipefail
...
# venv defaults: try the canonical setup_env.sh location first
# (/mnt/workspace/venvs), fall back to the legacy in-repo layout
# (${REPO_ROOT}/.{venv,teacherVenv}). Caller-provided TEACHER_VENV /
# ANALYSIS_VENV always wins.
...
RUN_DIR="${RUN_DIR:-${1:-}}"
if [[ -z "${RUN_DIR}" ]]; then
  echo "Usage: RUN_DIR=/path/to/run bash scripts/supplement_2rounds/G1.sh"
  echo "   or: bash scripts/supplement_2rounds/G1.sh /path/to/run"
  exit 1
fi
...
# more prompts before the next chunk's HOL barrier.
