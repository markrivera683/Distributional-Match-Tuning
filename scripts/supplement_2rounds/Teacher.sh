#!/usr/bin/env bash
# Teacher two-round post-eval via vLLM: 16k first pass, 32k retry on incorrect prompts.
# Usage:
#   bash scripts/supplement_2rounds/Teacher.sh /mnt/data/models/qwen3.5-27b
...
if [[ -z "${MODEL_PATH}" ]]; then
  echo "Usage: MODEL_PATH=/path/to/teacher bash scripts/supplement_2rounds/Teacher.sh"
  echo "   or: bash scripts/supplement_2rounds/Teacher.sh /path/to/teacher"
  exit 1
fi

TS="${TS:-$(date +%m%d_%H%M)}"
RUN_DIR="${RUN_DIR:-/root/outputs/teacher_2rounds_eval_${TS}}"
...
echo "========== Teacher Two-Round vLLM Eval =========="
