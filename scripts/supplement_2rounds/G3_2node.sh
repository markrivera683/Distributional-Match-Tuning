#!/usr/bin/env bash
# G3 two-round post-eval, 2-node parallel version.
#
# vs scripts/supplement_2rounds/G3.sh:
#   * G3.sh:     single-node TP=8 on the head node only (worker idle).
#   * G3_2node.sh: head and worker each run vLLM TP=8 in parallel on a
...
#     as run_G3_rebase_2node_once.sh.
...
#
# Usage:
#   bash scripts/supplement_2rounds/G3_2node.sh /mnt/data/.../outputs/g3_RUN
#   RUN_DIR=...   bash scripts/supplement_2rounds/G3_2node.sh
#
# Required env (typically set by run_G3_rebase_2node_once.sh):
...
# Fall back to single-node G3.sh if:
...
if [[ -n "${_fallback_to_single_node}" ]]; then
  echo "[G3_2node] ${_fallback_to_single_node}, falling back to single-node G3.sh"
  exec bash "${SCRIPT_DIR}/G3.sh" "$@"
fi
...
RUN_DIR="${RUN_DIR:-${1:-}}"
if [[ -z "${RUN_DIR}" ]]; then
  echo "Usage: RUN_DIR=/path/to/run bash scripts/supplement_2rounds/G3_2node.sh"
  echo "   or: bash scripts/supplement_2rounds/G3_2node.sh /path/to/run"
  exit 1
fi
...
echo "========== G3 Two-Round vLLM Eval (2-node parallel) =========="
