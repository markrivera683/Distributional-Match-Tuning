...
#   RUN_DIR=/root/outputs/g2_online_teacher_8gpu_0411_0652 bash scripts/supplement_2rounds/G2.sh
set -euo pipefail
...
RUN_DIR="${RUN_DIR:-${1:-}}"
if [[ -z "${RUN_DIR}" ]]; then
  echo "Usage: RUN_DIR=/path/to/run bash scripts/supplement_2rounds/G2.sh"
  echo "   or: bash scripts/supplement_2rounds/G2.sh /path/to/run"
  exit 1
fi
