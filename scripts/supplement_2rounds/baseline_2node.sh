#!/usr/bin/env bash
# Baseline two-round post-eval, 2-node parallel version.
#
# vs scripts/supplement_2rounds/baseline.sh:
#   * baseline.sh:     single-node TP=8 on the head node only (worker idle).
#   * baseline_2node.sh: head and worker each run vLLM TP=8 in parallel on a
...
#
# vs G3_2node.sh:
#   * No training, no teacher, no Ray. MODEL_PATH points to a base /
#     pretrained checkpoint (e.g. /mnt/data/models/Qwen3.5-4B/) or to a
#     ckpt produced by an earlier training run.
#   * MODEL_PATH must be explicitly provided by the launcher (no
#     RUN_DIR/model fallback, because baseline does not train).
#
# Why dataset-split parallel vs cross-node TP=16:
#   * Sub-10B models already saturate a single A100 box at TP=8;
...
#     MODEL_PATH / EVAL_DATA / LOG_DIR are all on the shared NAS).
#   * Passwordless SSH from head -> worker (DSW), or DLC mode where the
#     worker pod is reachable via rendezvous file polling. Same plumbing
#     as run_baseline_2node_once.sh.
#   * Each node has 8 GPUs available for the post-eval stage.
#
# Usage:
#   bash scripts/supplement_2rounds/baseline_2node.sh /path/to/run
#   RUN_DIR=...   bash scripts/supplement_2rounds/baseline_2node.sh
#
# Required env (typically set by run_baseline_2node_once.sh):
#   MODEL_PATH          base / pretrained ckpt path (or trained ckpt path)
#   WORKER_SSH_TARGET   user@host (or host) for ssh to worker (DSW only)
#   SSH_OPTS            optional ssh options string (DSW only)
...
#                                  baseline.sh behavior (default: true)
...
# Fall back to single-node baseline.sh if:
...
if [[ -n "${_fallback_to_single_node}" ]]; then
  echo "[baseline_2node] ${_fallback_to_single_node}, falling back to single-node baseline.sh"
...
RUN_DIR="${RUN_DIR:-${1:-}}"
if [[ -z "${RUN_DIR}" ]]; then
  echo "Usage: RUN_DIR=/path/to/run bash scripts/supplement_2rounds/baseline_2node.sh"
  echo "   or: bash scripts/supplement_2rounds/baseline_2node.sh /path/to/run"
  exit 1
fi

# baseline does not produce a trained ckpt under RUN_DIR/model, so we
# require the launcher to set MODEL_PATH explicitly (e.g.
# /mnt/data/models/Qwen3.5-4B/ or a previously trained ckpt path).
MODEL_PATH="${MODEL_PATH:?MODEL_PATH must be set (no training-produced ckpt for baseline)}"
...
# the original ordering by source_idx (matches G3.sh's
...
# complete (matches G3.sh's _stage_output_already_complete behavior).
run_shard_local() {
  local stage_name="$1"
...
echo "========== Baseline Two-Round vLLM Eval (2-node parallel) =========="
