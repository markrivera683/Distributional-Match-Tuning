...
# REPO_ROOT auto-derived from this script's location so the worker is
# portable across DSW (/root/code/Distributional-Match-Tuning -> /mnt/data
# symlink) and DLC (no symlink, repo lives directly on the OSS mount).
# Override only if you've physically moved this script outside its repo.
...
# are factored out into _vllm_runtime.sh so they stay in sync across baseline,
# G1, G2, G3 and Teacher worker scripts.
