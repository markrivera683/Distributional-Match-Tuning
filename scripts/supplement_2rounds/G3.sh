#!/usr/bin/env bash
# G3 two-round post-eval via vLLM: 16k first pass, 32k retry on incorrect prompts.
# Usage:
#   bash scripts/supplement_2rounds/G3.sh /root/outputs/g3_online_teacher_8gpu_0411_0652
#   RUN_DIR=/root/outputs/g3_online_teacher_8gpu_0411_0652 bash scripts/supplement_2rounds/G3.sh
set -euo pipefail
...
RUN_DIR="${RUN_DIR:-${1:-}}"
if [[ -z "${RUN_DIR}" ]]; then
  echo "Usage: RUN_DIR=/path/to/run bash scripts/supplement_2rounds/G3.sh"
  echo "   or: bash scripts/supplement_2rounds/G3.sh /path/to/run"
  exit 1
fi
