#!/usr/bin/env bash
# G1 two-round post-eval, 2-node parallel version.
#
# vs scripts/supplement_2rounds/G1.sh:
#   * G1.sh:     single-node TP=8 on the head node only (worker idle).
#   * G1_2node.sh: head and worker each run vLLM TP=8 in parallel on a
...
#     as run_G1_rebase_2node_once.sh.
#   * Each node has 8 GPUs available for the post-eval stage (training
#     ray cluster has already been torn down by the launcher's cleanup
#     trap before this script runs).
#
# Usage:
#   bash scripts/supplement_2rounds/G1_2node.sh /mnt/data/.../outputs/g1_RUN
#   RUN_DIR=...   bash scripts/supplement_2rounds/G1_2node.sh
#
# Required env (typically set by run_G1_rebase_2node_once.sh):
...
# Fall back to single-node G1.sh if:
...
if [[ -n "${_fallback_to_single_node}" ]]; then
  echo "[G1_2node] ${_fallback_to_single_node}, falling back to single-node G1.sh"
  exec bash "${SCRIPT_DIR}/G1.sh" "$@"
fi
...
RUN_DIR="${RUN_DIR:-${1:-}}"
if [[ -z "${RUN_DIR}" ]]; then
  echo "Usage: RUN_DIR=/path/to/run bash scripts/supplement_2rounds/G1_2node.sh"
  echo "   or: bash scripts/supplement_2rounds/G1_2node.sh /path/to/run"
  exit 1
fi

# G1 produces a trained ckpt under RUN_DIR/model, so default MODEL_PATH
# to that. The launcher may also override to a different ckpt path.
...
# the original ordering by source_idx (matches G1.sh's
...
# complete (matches G1.sh's _stage_output_already_complete behavior).
run_shard_local() {
  local stage_name="$1"
...
echo "========== G1 Two-Round vLLM Eval (2-node parallel) =========="
