...
REPO_ROOT="${REPO_ROOT:-/root/autodl-tmp/Energy/ebft_openrlhf_u1_ncfm}"

# Paper-aligned target model for the Q&A code setting.
MODEL_PATH="${MODEL_PATH:-/root/autodl-tmp/modelscope_cache_test/Qwen/Qwen2___5-1___5B}"
...
RUN_TAG="${RUN_TAG:-step1_paperqa_${FEATURE_MAP_TYPE}_qwen25_1p5b_seed${GLOBAL_SEED}}"
RUN_ROOT="${RUN_ROOT:-/root/autodl-tmp/Energy/ebft_openrlhf_u1_ncfm/runs/${RUN_TAG}}"
...
if [[ ! -d "${MODEL_PATH}" ]]; then
  ...
  echo "Set MODEL_PATH to a complete local copy of Qwen2.5-1.5B before running."
  exit 2
fi

if ! has_model_weights; then
  echo "MODEL_PATH exists but no model weight files were found: ${MODEL_PATH}"
  echo "The paper-aligned Q&A baseline expects a complete Qwen2.5-1.5B checkpoint."
  echo "Current path appears to contain config/tokenizer files only."
  exit 3
fi

echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] Starting paper-aligned Q&A feature-map run"
echo "REPO_ROOT=${REPO_ROOT}"
echo "MODEL_PATH=${MODEL_PATH}"
echo "FEATURE_MAP_TYPE=${FEATURE_MAP_TYPE}"
echo "RFF_NUM_FEATURES=${RFF_NUM_FEATURES}"
echo "RFF_SIGMA=${RFF_SIGMA}"
echo "RFF_SEED=${RFF_SEED}"
echo "DISTRIBUTION_REWARD_TYPE=${DISTRIBUTION_REWARD_TYPE}"
echo "CF_NUM_FREQS=${CF_NUM_FREQS}"
echo "CF_SIGMA=${CF_SIGMA}"
echo "CF_SEED=${CF_SEED}"
echo "CF_ALPHA=${CF_ALPHA}"
echo "CF_BETA=${CF_BETA}"
echo "CF_REWARD_SCALE=${CF_REWARD_SCALE}"
...
echo "CRITIC_SEQUENCE_LEVEL=${CRITIC_SEQUENCE_LEVEL}"
echo "FEATURE_ADAPTER_ENABLE=${FEATURE_ADAPTER_ENABLE}"
echo "FEATURE_ADAPTER_TYPE=${FEATURE_ADAPTER_TYPE}"
echo "FEATURE_ADAPTER_RANK=${FEATURE_ADAPTER_RANK}"
echo "FEATURE_ADAPTER_DROPOUT=${FEATURE_ADAPTER_DROPOUT}"
echo "CRITIC_DIRECT_DISCREPANCY_COEF=${CRITIC_DIRECT_DISCREPANCY_COEF}"
echo "CRITIC_DIRECT_DISCREPANCY_TARGET=${CRITIC_DIRECT_DISCREPANCY_TARGET}"
echo "CE_LOSS_COEF=${CE_LOSS_COEF}"
echo "DIVERSITY_REW_COEF=${DIVERSITY_REW_COEF}"
echo "ALIGNMENT_REW_COEF=${ALIGNMENT_REW_COEF}"
...
echo "EVAL_DOWN_BATCH_SIZE=${EVAL_DOWN_BATCH_SIZE}"
echo "EVAL_GENERATE_MAX_LEN=${EVAL_GENERATE_MAX_LEN}"
echo "EVAL_N_SAMPLES_PER_PROMPT=${EVAL_N_SAMPLES_PER_PROMPT}"
echo "EVAL_N_SAMPLES_PER_PROMPT_DOWN=${EVAL_N_SAMPLES_PER_PROMPT_DOWN}"
echo "OPENRLHF_RAY_OBJECT_STORE_MEMORY_BYTES=${OPENRLHF_RAY_OBJECT_STORE_MEMORY_BYTES}"
echo "ENABLE_GRADIENT_CHECKPOINTING=${ENABLE_GRADIENT_CHECKPOINTING}"
echo "GRADIENT_CHECKPOINTING_USE_REENTRANT=${GRADIENT_CHECKPOINTING_USE_REENTRANT}"
echo "EMA_BETA=${EMA_BETA}"

echo "Paper-aligned values:"
echo "  prompt_data=sjelassi/opencode-instruct_100k_200tok"
echo "  prompt_max_len=1024 stride=8 context_max_len=8 generate_max_len=8"
echo "  rollout_batch_size=16 train_batch_size=64 micro_*_batch_size=8"
echo "  temperature=0.6 n_samples_per_prompt=4 actor_learning_rate=1e-6"
echo "Single-GPU compatibility deviations:"
echo "  colocate_all_models=true"
echo "  adam_offload=true"
echo "  flash_attn=true"
echo "  use_tensorboard enabled"
echo "  Ray object store capped for single-node reproduction"
...
echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] Paper-aligned Q&A feature-map run finished"
