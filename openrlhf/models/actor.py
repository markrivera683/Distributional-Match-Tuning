...
class Actor(nn.Module):
    ...
    def get_attention_implementation(self) -> Optional[str]:
        target = _unwrap_attention_target(self.model)
        return getattr(target.config, "_attn_implementation", None)

    def set_attention_implementation(self, value: Optional[str]) -> None:
        target = _unwrap_attention_target(self.model)
        target.config._attn_implementation = value

    def _debug_validate_forward_inputs(
        self,
        sequences: torch.Tensor,
        attention_mask: Optional[torch.Tensor],
        position_ids: Optional[torch.Tensor],
    ) -> None:
        """Validate actor forward inputs under an opt-in debug flag.

        This is purely for failure localization and should not affect normal runs.
        """
        if not bool(int(os.environ.get("OPENRLHF_DEBUG_ACTOR_INPUTS", "0"))):
            return

        vocab_size = getattr(self.model.config, "vocab_size", None)
        seq_min = int(sequences.min().item())
        seq_max = int(sequences.max().item())
        if seq_min < 0:
            raise ValueError(f"Actor forward received negative token id: min={seq_min}")
        if vocab_size is not None and seq_max >= vocab_size:
            raise ValueError(
                f"Actor forward received token id >= vocab_size: max={seq_max}, vocab_size={vocab_size}"
            )
        ...
        if position_ids is not None:
            ...
            if pos_min < 0:
                raise ValueError(f"Actor forward received negative position id: min={pos_min}")
            if position_ids.shape != sequences.shape:
                raise ValueError(
                    f"Actor forward position_ids shape {tuple(position_ids.shape)} "
                    f"does not match sequences shape {tuple(sequences.shape)}"
                )
        else:
        ...
        if attention_mask is not None:
            if attention_mask.dim() == 4:
                if attention_mask.shape[0] != sequences.shape[0] or attention_mask.shape[2] != sequences.shape[1] or attention_mask.shape[3] != sequences.shape[1]:
                    raise ValueError(
                        f"Actor forward 4D attention mask shape {tuple(attention_mask.shape)} "
                        f"is incompatible with sequences shape {tuple(sequences.shape)}"
                    )
            elif attention_mask.dim() == 2:
                if attention_mask.shape != sequences.shape:
                    raise ValueError(
                        f"Actor forward 2D attention mask shape {tuple(attention_mask.shape)} "
                        f"does not match sequences shape {tuple(sequences.shape)}"
                    )
            if not torch.isfinite(attention_mask).all():
                finite = torch.isfinite(attention_mask)
                raise ValueError(
                    f"Actor forward attention mask contains non-finite values: "
                    f"{int((~finite).sum().item())} entries"
                )

        logger.info(
            "Actor forward debug: sequences=%s dtype=%s token_min=%d token_max=%d "
            "pos_min=%s pos_max=%s attention_mask_shape=%s attention_mask_dim=%s",
            ...
            attention_mask.dim() if attention_mask is not None else None,
        )

    def _debug_validate_logprob_inputs(
        self,
        ...
        """Validate post-forward tensors before any gather/index ops.

        This is opt-in debug logic only and should not affect normal runs.
        """
        if not bool(int(os.environ.get("OPENRLHF_DEBUG_ACTOR_INPUTS", "0"))):
            return

        if processed_logits.dim() != 3:
            raise ValueError(f"processed_logits must be 3D, got shape={tuple(processed_logits.shape)}")
        if target_labels.dim() != 2:
            raise ValueError(f"target_labels must be 2D, got shape={tuple(target_labels.shape)}")
        if processed_logits.shape[:2] != target_labels.shape:
            raise ValueError(
                f"processed_logits/time shape {tuple(processed_logits.shape[:2])} "
                f"does not match target_labels shape {tuple(target_labels.shape)}"
            )

        vocab_size = processed_logits.shape[-1]
        label_min = int(target_labels.min().item())
        label_max = int(target_labels.max().item())
        if label_min < 0:
            raise ValueError(f"target_labels contains negative ids: min={label_min}")
        if label_max >= vocab_size:
            raise ValueError(
                f"target_labels contains id >= vocab size: max={label_max}, vocab_size={vocab_size}"
            )
        if not torch.isfinite(processed_logits).all():
            finite = torch.isfinite(processed_logits)
            raise ValueError(
                f"processed_logits contains non-finite values: {int((~finite).sum().item())} entries"
            )

        logger.info(
            "Actor logprob debug: logits=%s labels=%s vocab=%d label_min=%d label_max=%d "
            "prompt_len=%s context_len=%s num_blocks=%s stride=%s",
            tuple(processed_logits.shape),
            tuple(target_labels.shape),
            vocab_size,
            label_min,
            label_max,
            prompt_len,
    ...
    def forward(
        ...
        if (
            forward_attention_mask is not None
            and forward_attention_mask.dim() == 4
        ...
        self._debug_validate_forward_inputs(sequences, forward_attention_mask, position_ids)
        ...
        self._debug_validate_logprob_inputs(
            processed_logits,
            target_labels,
            prompt_len,
            context_len,
