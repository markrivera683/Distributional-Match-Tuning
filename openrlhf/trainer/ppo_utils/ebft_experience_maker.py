...
class RemoteExperienceMaker(ABC):
    ...
    def _get_remote_teacher_samples(
        ...
        """Build teacher Experience objects from per-block remote API completions.

        For each prompt, every block k has a distinct context window
        ``prompt[k*stride : k*stride + context_length]``.  We send each
        block-specific context to the remote DLC teacher service and obtain
        M independent completions.  The resulting token ids are placed in
        the interleaved format that the critic's strided attention expects,
        so each block position contains tokens genuinely generated by the
        remote model for *that* block's context.
        """
        ...
        if teacher_max_tokens is None:
            teacher_max_tokens = generate_length
        ...
        # ── 1. Build per-block context texts ──────────────────────────
        #   block k sees prompt[k*stride : k*stride + context_length]
        block_context_texts = []   # length = P * K
        block_index_map = []       # (prompt_idx, block_idx) for each entry
        for p_idx, prompt_tensor in enumerate(unique_prompts):
            for block_idx in range(K):
                start = block_idx * stride
                end = start + context_length
                ctx_ids = prompt_tensor[start:end]
                ctx_text = self.tokenizer.decode(ctx_ids, skip_special_tokens=True)
                block_context_texts.append(ctx_text)
                block_index_map.append((p_idx, block_idx))

        logger.info(
            "[Teacher-Remote] Requesting %d block contexts (P=%d x K=%d) x M=%d "
            "samples from DLC remote teacher (model=%s) ...",
            len(block_context_texts), P, K, M,
            getattr(self.teacher_provider, "model_name", "?"),
        )

        # ── 2. Call remote DLC teacher service ────────────────────────
        all_completions = self.teacher_provider.sample_targets(
            block_context_texts, M, teacher_temp, teacher_top_p, teacher_max_tokens,
        )
        # all_completions[i][m] = completion text for block_context_texts[i], sample m

        # ── 3. Verification: completions are from remote response ─────
        sample_idx = 0
        sample_text = all_completions[sample_idx][0] if all_completions else ""
        logger.info(
            "[Teacher-Remote] ✓ Completions received from remote DLC teacher.\n"
            "  backend        = remote\n"
            "  remote_model   = %s\n"
            "  block_contexts = %d  (P=%d, K=%d)\n"
            "  samples_per_ctx= %d\n"
            "  example_comp   = %r",
            getattr(self.teacher_provider, "model_name", "?"),
            len(block_context_texts), P, K, M,
            sample_text[:120] + ("..." if len(sample_text) > 120 else ""),
        )

        # ── 4. Tokenize and build per-block interleaved full_sequences ─
        pad_id = self.tokenizer.pad_token_id
        if pad_id is None:
            pad_id = self.tokenizer.eos_token_id or 0

        # Organize: comp_tokens[p_idx][m_idx][block_idx] = Tensor(G,)
        comp_tokens = [[None] * M for _ in range(P)]
        for flat_idx, (p_idx, block_idx) in enumerate(block_index_map):
            for m_idx in range(M):
                comp_text = all_completions[flat_idx][m_idx]
                ids = self.tokenizer.encode(comp_text, add_special_tokens=False)
                ids = ids[:G]
                if len(ids) < G:
                    ids = ids + [pad_id] * (G - len(ids))
                t = torch.tensor(ids, dtype=torch.long)
                if comp_tokens[p_idx][m_idx] is None:
                    comp_tokens[p_idx][m_idx] = [None] * K
                comp_tokens[p_idx][m_idx][block_idx] = t

        teacher_samples = []
        for p_idx in range(P):
            prompt_ids = unique_prompts[p_idx]
            p_doc_ids = unique_doc_ids[p_idx]
            p_qa_masks = unique_qa_masks[p_idx]

            full_seqs = []
            for m_idx in range(M):
                block_list = comp_tokens[p_idx][m_idx]  # list of K tensors, each (G,)
                stacked = torch.stack(block_list)        # (K, G)
                # Interleave: gen[s*K + k] = stacked[k, s]
                # stacked.t() → (G, K); reshape(-1) → [b0_s0, b1_s0, ..., bK_s0, b0_s1, ...]
                gen_region = stacked.t().reshape(-1)     # (G*K,)
        ...
        logger.info(
            "[Teacher-Remote] Built %d teacher sample batches, "
            "each full_sequences shape %s  "
            "(per-block completions from remote DLC teacher, NOT replicated)",
