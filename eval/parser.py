...
def parse_ground_truth(example: Dict[str, Any], data_name):
    if "gt_cot" in example and "gt" in example:
        ...
        elif data_name.startswith("aops") and isinstance(example["gt"], list):
            gt_ans = [x if x.startswith("json") else strip_string(x) for x in example["gt"] ]
    ...
    elif data_name in ["math", "minerva_math"]:
        gt_cot = example["solution"]
...
def parse_question(example, data_name):
    ...
    elif data_name.startswith("omni_math"):
