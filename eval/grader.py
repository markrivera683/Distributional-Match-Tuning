...
from formalize_ans import standardize_answer, format_output
...
def math_equal(
    ...
    reference: Union[float, str, list],
    ...
    if isinstance(reference, list):
        for x in reference:
            if math_equal(prediction, x, include_percentage, is_close, timeout):
                return True
        return False
    ...
    try:
        prediction_aops = format_output(standardize_answer(prediction))
        if symbolic_equal(prediction_aops, reference):
            return True
        if aops_is_list and isinstance(prediction, list):
            reference = json.loads(reference)
            reference_list = []
            for element in reference:
                if isinstance(element, list):
                    reference_list.append(str(element))
                else:
                    reference_list.append(element)
            reference = sorted(reference_list)
            
            prediction_list = []
            for element in prediction:
                if isinstance(element, list):
                    prediction_list.append(str(element))
                else:
                    prediction_list.append(element)
            prediction = sorted(prediction_list)
        ...
        print("error trying to parse like aops", reference)
        print(e)
