...
class RemoteExperienceMaker(ABC):
    def __init__(
        ...
        **kwargs,
        ...
        self.args = strategy.args
        self.raw_question_texts = raw_question_texts or []
    ...
    @torch.no_grad()
    def _get_remote_teacher_samples(
        self,
        unique_prompts,
        unique_doc_ids,
        unique_qa_masks,
        ...
        """Build teacher Experience objects from full-question remote API completions.

        Instead of sending each block's 8-token fragment to the teacher, this
        method extracts the **full question text** for each QA pair (using
        ``doc_ids`` and ``raw_question_texts``), calls the teacher **once per
        unique question**, then maps the teacher's full answer back into the
        block-interleaved token layout that the downstream critic expects.

        This ensures the teacher sees complete math problems and produces
        semantically meaningful answers, while the student-side block
        architecture is unchanged.
        """
        assert self.teacher_provider is not None, (
            "teacher_provider must be set for remote teacher path"
        )
        assert self.teacher_samples_generator is None, (
            "teacher_samples_generator must be None when teacher_backend=remote — "
            "local teacher model must not participate in completion generation"
        )

        teacher_temp = float(getattr(self.args, "teacher_temperature", 0.7))
        teacher_top_p = float(getattr(self.args, "teacher_top_p", 0.95))
        teacher_max_tokens = getattr(self.args, "teacher_max_new_tokens", None)
        if teacher_max_tokens is None:
        ...
        # ── 1. Collect unique questions across all chunks ───────────────
        #   Each packed chunk contains tokens from one or more QA pairs
        #   identified by doc_ids. We look up the full question text for
        #   each doc_id from self.raw_question_texts (indexed by doc_id).
        all_question_doc_ids = set()
        for p_idx in range(P):
            doc_id_tensor = unique_doc_ids[p_idx]
            for did in doc_id_tensor.unique().tolist():
                if did >= 0:
                    all_question_doc_ids.add(int(did))

        has_raw_questions = bool(self.raw_question_texts)

        question_texts = {}  # doc_id -> question string
        for did in sorted(all_question_doc_ids):
            if has_raw_questions and did < len(self.raw_question_texts):
                question_texts[did] = self.raw_question_texts[did]
            else:
                # Fallback: decode question tokens from the first chunk
                # that contains this doc_id's question portion
                q_text = self._decode_question_from_chunks(
                    did, unique_prompts, unique_doc_ids, unique_qa_masks,
                )
                question_texts[did] = q_text

        unique_questions = list(question_texts.items())  # [(doc_id, text), ...]
        prompts_for_teacher = [text for _, text in unique_questions]

        logger.info(
            "[Teacher-FullQ] Requesting %d unique questions x M=%d samples "
            "from remote teacher (model=%s).\n"
            "  example question = %r",
            len(prompts_for_teacher), M,
            getattr(self.teacher_provider, "model_name", "?"),
            (prompts_for_teacher[0][:150] + "...") if prompts_for_teacher else "<none>",
        )

        # ── 2. Call remote teacher with FULL question texts ────────────
        all_completions = self.teacher_provider.sample_targets(
            prompts_for_teacher, M, teacher_temp, teacher_top_p, teacher_max_tokens,
        )
        # all_completions[q_idx][m] = full answer text

        teacher_answers = {}  # doc_id -> List[List[int]] (M tokenized answers)
        for q_idx, (did, _) in enumerate(unique_questions):
            tokenized_answers = []
            for m_idx in range(M):
                ans_text = all_completions[q_idx][m_idx]
                ans_ids = self.tokenizer.encode(ans_text, add_special_tokens=False)
                tokenized_answers.append(ans_ids)
            teacher_answers[did] = tokenized_answers

        sample_ans = all_completions[0][0] if all_completions else ""
        logger.info(
            "[Teacher-FullQ] ✓ Full-question completions received.\n"
            "  unique_questions = %d\n"
            "  samples_per_q    = %d\n"
            "  example_answer   = %r",
            len(unique_questions), M,
            (sample_ans[:200] + "...") if len(sample_ans) > 200 else sample_ans,
        )

        # ── 3. For each chunk, build teacher prompt and block tokens ───
        teacher_samples = []
        for p_idx in range(P):
            prompt_ids = unique_prompts[p_idx]       # (prompt_length,)
            p_doc_ids = unique_doc_ids[p_idx]         # (prompt_length,)
            p_qa_masks = unique_qa_masks[p_idx]       # (prompt_length,)

            teacher_prompt = self._build_teacher_prompt(
                prompt_ids, p_doc_ids, p_qa_masks,
                teacher_answers, M, pad_id,
            )
            # teacher_prompt: (M, prompt_length)

            full_seqs = []
            for m_idx in range(M):
                t_prompt = teacher_prompt[m_idx]  # (prompt_length,)
                block_tokens = []  # K tensors of shape (G,)
                for block_idx in range(K):
                    start = block_idx * stride + context_length
                    end = start + G
                    if end <= t_prompt.shape[0]:
                        block_tok = t_prompt[start:end]
                    else:
                        avail = max(0, t_prompt.shape[0] - start)
                        if avail > 0:
                            block_tok = torch.cat([
                                t_prompt[start:start + avail],
                                torch.full((G - avail,), pad_id, dtype=torch.long),
                            ])
                        else:
                            block_tok = torch.full((G,), pad_id, dtype=torch.long)
                    block_tokens.append(block_tok)

                stacked = torch.stack(block_tokens)  # (K, G)
                gen_region = stacked.t().reshape(-1)  # (G*K,) interleaved
                full_seqs.append(torch.cat([prompt_ids, gen_region]))

            batch_full_seqs = torch.stack(full_seqs)  # (M, prompt_length + G*K)

            strided_doc = (
                p_doc_ids[context_length:]
                .unfold(0, G, stride).t().reshape(-1)
            )
            batch_doc_ids = torch.cat([p_doc_ids, strided_doc]).unsqueeze(0).expand(M, -1)

            strided_qa = (
                p_qa_masks[context_length:]
                .unfold(0, G, stride).t().reshape(-1)
            )
            batch_qa_masks = torch.cat([p_qa_masks, strided_qa]).unsqueeze(0).expand(M, -1)

            teacher_samples.append(
                Experience(
                    prompts=prompt_ids.unsqueeze(0).expand(M, -1),
                    full_sequences=batch_full_seqs,
        ...
        logger.info(
            "[Teacher-FullQ] Built %d teacher sample batches, "
            "each full_sequences shape %s  "
            "(full-question teacher answers, block-aligned)",
            len(teacher_samples), teacher_samples[0].full_sequences.shape,
        )
        return teacher_samples

    def _decode_question_from_chunks(
        self, doc_id, unique_prompts, unique_doc_ids, unique_qa_masks,
    ):
        """Fallback: decode question tokens for a given doc_id from the packed chunks."""
        q_token_ids = []
        for p_idx in range(len(unique_prompts)):
            did_tensor = unique_doc_ids[p_idx]
            qa_mask = unique_qa_masks[p_idx]
            prompt = unique_prompts[p_idx]
            mask = (did_tensor == doc_id) & (qa_mask == 0)
            if mask.any():
                q_token_ids.extend(prompt[mask].tolist())
        if q_token_ids:
            return self.tokenizer.decode(q_token_ids, skip_special_tokens=True)
        # Last resort: decode the first chunk that has this doc_id
        for p_idx in range(len(unique_prompts)):
            did_tensor = unique_doc_ids[p_idx]
            prompt = unique_prompts[p_idx]
            mask = did_tensor == doc_id
            if mask.any():
                return self.tokenizer.decode(prompt[mask].tolist(), skip_special_tokens=True)
        return ""

    def _build_teacher_prompt(
        self, prompt_ids, doc_ids, qa_masks,
        teacher_answers, M, pad_id,
    ):
        """Build M teacher-prompt variants for one packed chunk.

        For each teacher sample m, the teacher prompt has the same question
        tokens as the GT but the answer region is filled with the teacher's
        tokenized answer (truncated/padded to fit).

        Returns: Tensor (M, prompt_length)
        """
        prompt_length = prompt_ids.shape[0]
        result = prompt_ids.unsqueeze(0).expand(M, -1).clone()  # (M, L)

        present_doc_ids = doc_ids.unique().tolist()
        present_doc_ids = [d for d in present_doc_ids if d >= 0]

        total_replaced = 0
        for did in present_doc_ids:
            if did not in teacher_answers:
                continue

            doc_mask = doc_ids == did
            ans_mask = (doc_mask) & (qa_masks == 1)
            ans_positions = ans_mask.nonzero(as_tuple=True)[0]

            if ans_positions.numel() == 0:
                continue

            ans_start = ans_positions[0].item()
            ans_len = ans_positions.numel()

            for m_idx in range(M):
                t_ans_ids = teacher_answers[did][m_idx]
                t_ans_len = len(t_ans_ids)

                fill_len = min(ans_len, t_ans_len)
                if fill_len > 0:
                    old_tokens = result[m_idx, ans_start:ans_start + fill_len].clone()
                    result[m_idx, ans_start:ans_start + fill_len] = torch.tensor(
                        t_ans_ids[:fill_len], dtype=torch.long,
                    )
                    new_tokens = result[m_idx, ans_start:ans_start + fill_len]
                    diff_count = (old_tokens != new_tokens).sum().item()
                    total_replaced += fill_len
                    if m_idx == 0:
                        logger.info(
                            "[Teacher-Diag] doc_id=%d m=%d: replaced %d/%d answer tokens "
                            "(pos %d..%d), %d tokens actually changed",
                            did, m_idx, fill_len, ans_len,
                            ans_start, ans_start + fill_len, diff_count,
                        )
                if t_ans_len < ans_len:
                    result[m_idx, ans_start + t_ans_len:ans_start + ans_len] = pad_id

        if total_replaced == 0:
            logger.warning("[Teacher-Diag] WARNING: zero tokens replaced by teacher in this chunk")
    ...
    def _build_teacher_embedding(
        ...
        Supports two backends:
          - local (teacher_samples_generator): existing strided-block generation
          - remote (teacher_provider): HTTP API completions → local tokenize
        ...
        # ── 2. Get teacher samples (local or remote — mutually exclusive) ──
        _backend = getattr(self.args, "teacher_backend", "local")
        if self.teacher_provider is not None:
            assert _backend == "remote", (
                f"teacher_provider is set but teacher_backend={_backend!r}; "
                "provider must only be used when teacher_backend=remote"
            )
            assert self.teacher_samples_generator is None, (
                "When teacher_backend=remote, local teacher_samples_generator "
                "must be None — local teacher model must not participate"
            )
            teacher_samples = self._get_remote_teacher_samples(
                unique_prompts, unique_doc_ids, unique_qa_masks,
                M, prompt_length, context_length, generate_length, stride, num_blocks,
            )
        elif self.teacher_samples_generator is not None:
            assert _backend == "local", (
                f"teacher_samples_generator is set but teacher_backend={_backend!r}; "
                "local generator must only be used when teacher_backend=local"
            )
            logger.info(
                "[Teacher] Generating %d local teacher samples for %d unique prompts ...",
                M, len(unique_prompts),
            )
            try:
                ...
                logger.error(f"[Teacher] Local teacher generation FAILED: {e}")
                return None
        else:
            return None

        if not teacher_samples:
            logger.warning("[Teacher] Teacher generation returned empty list")
            return None
        logger.info(
            "[Teacher] Got %d teacher sample batches, each with %s",
            len(teacher_samples), teacher_samples[0].full_sequences.shape,
        )
        ...
        emb_hash = hash(gen_emb.float().sum().item())
        logger.info(
            f"[Teacher-Diag] teacher_embedding stats: mean={gen_emb.float().mean().item():.8f}, "
            f"std={gen_emb.float().std().item():.8f}, hash={emb_hash}"
        )
        return gen_emb
    ...
    def make_experience(self, samples_list):
        ...
        _has_tp = self.teacher_provider is not None
        _has_teacher = _has_tsg or _has_tp
        logger.info(
            "[TEACHER-VERIFY] Teacher branch gate: "
            "distribution_reward_type=%s, cf_target_mode=%s, "
            "teacher_samples_generator=%s, teacher_provider=%s "
            "=> %s",
            _dr_type, _ct_mode,
            'PRESENT' if _has_tsg else 'None',
            'PRESENT' if _has_tp else 'None',
        ...
        if (
            ...
            _tb = getattr(self.args, "teacher_backend", "local")
            if teacher_embedding is not None:
                logger.info(
                    "[TEACHER-VERIFY] teacher_embedding built: "
                    "backend=%s, shape=%s, dtype=%s, mean=%.6f  "
                    "(completions sourced from %s)",
                    _tb, teacher_embedding.shape, teacher_embedding.dtype,
                    teacher_embedding.float().mean().item(),
                    "REMOTE DLC teacher API" if _tb == "remote" else "local teacher model",
                )
            else:
        ...
        logger.info(
            f"[TEACHER-DIAG] === Reward construction summary ===\n"
            f"  cf_target_mode       = {getattr(self.args, 'cf_target_mode', 'N/A')}\n"
            f"  teacher enabled      = {teacher_embedding is not None}\n"
            f"  teacher sample count = {teacher_embedding.shape[2] if teacher_embedding is not None else 0}\n"
            f"  teacher lambda       = {getattr(self.args, 'cf_teacher_lambda', 0.0)}\n"
            f"  gen_embedding.shape  = {gen_embedding.shape}\n"
            f"  gt_embedding.shape   = {gt_embedding.shape}\n"
            f"  teacher_emb.shape    = {teacher_embedding.shape if teacher_embedding is not None else 'None'}\n"
        ...
        logger.info(
            f"[TEACHER-DIAG] === Final reward stats ===\n"
            f"  gt_rewards   shape={gt_rewards_tensor.shape}  mean={gt_rewards_tensor.float().mean().item():.6f}  std={gt_rewards_tensor.float().std().item():.6f}\n"
            f"  div_rewards  shape={diversity_rewards_tensor.shape}  mean={diversity_rewards_tensor.float().mean().item():.6f}\n"
            f"  teacher_backend   = {getattr(self.args, 'teacher_backend', 'local')}\n"
            f"  teacher_in_reward = {teacher_embedding is not None and getattr(self.args, 'cf_target_mode', 'single') == 'teacher'}"
        )
