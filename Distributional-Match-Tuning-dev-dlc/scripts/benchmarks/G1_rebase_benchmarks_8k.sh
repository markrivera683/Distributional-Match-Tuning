#!/usr/bin/env bash
# 8k benchmark completion suite for a G1 rebase checkpoint.
#
# Usage:
#   RUN_DIR=/root/outputs/g1_rebase_xxx bash scripts/benchmarks/G1_rebase_benchmarks_8k.sh
#   bash scripts/benchmarks/G1_rebase_benchmarks_8k.sh /root/outputs/g1_rebase_xxx
set -euo pipefail
...
RUN_DIR="${RUN_DIR:-${1:-}}"
if [[ -z "${RUN_DIR}" ]]; then
  echo "Usage: RUN_DIR=/path/to/run bash scripts/benchmarks/G1_rebase_benchmarks_8k.sh" >&2
  echo "   or: bash scripts/benchmarks/G1_rebase_benchmarks_8k.sh /path/to/run" >&2
  exit 1
fi

MODEL_LABEL="${MODEL_LABEL:-g1_rebase}"
