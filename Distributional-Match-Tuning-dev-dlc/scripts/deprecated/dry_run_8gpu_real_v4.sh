...
# 8卡 dry-run: 4+4 拓扑，修正参数 (prompt_max_len >= generate_max_len + context_max_len)
python3 -m openrlhf.cli.train_ebft_ray \
  ...
  --prompt_max_len 768 --context_max_len 512 --generate_max_len 128 \
...
echo "" | tee -a "$RUN_DIR/dryrun.log"
echo "=== Training Progress ===" | tee -a "$RUN_DIR/dryrun.log"
tail -30 "$RUN_DIR/dryrun.log" | tee -a "$RUN_DIR/dryrun.log"

# 等待训练完成
echo "" | tee -a "$RUN_DIR/dryrun.log"
echo "Waiting for training to complete..." | tee -a "$RUN_DIR/dryrun.log"
wait $PID || true
