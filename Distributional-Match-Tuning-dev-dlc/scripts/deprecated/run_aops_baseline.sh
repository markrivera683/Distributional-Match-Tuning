...
# AoPS Baseline Script - Pointwise Reward
# Usage: MAX_SAMPLES=64 NUM_EPISODES=1 bash scripts/run_aops_baseline.sh
...
# Reward Type: pointwise for baseline
...
# CF Settings (used even for pointwise baseline for consistency)
...
RUN_TAG="${RUN_TAG:-aops_baseline_${DISTRIBUTION_REWARD_TYPE}_seed${GLOBAL_SEED}}"
...
echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] Starting AoPS baseline run"
...
echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] AoPS baseline run finished"
